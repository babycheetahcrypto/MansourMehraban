            ...
            <div>
              <h2 className="font-bold text-sm text-white">
                {`${user.firstName || ''} ${user.lastName || ''}`.trim().slice(0, 12) +
                  (`${user.firstName || ''} ${user.lastName || ''}`.trim().length > 12
                    ? '...'
                    : '')}
              </h2>
  ...
  const renderHome = () => (
    <div className="flex-grow flex flex-col items-center justify-center p-4 pb-16 relative">
      <div className="text-center mb-4 w-full max-w-md">
        ...
        <div className="flex flex-col items-center justify-center w-full max-w-[300] mx-auto mb-16">
          <button
            className="w-[calc(100vw-8px)] max-w-[500px] aspect-square rounded-full overflow-hidden shadow-lg z-20 coin-button mb-6 transform transition-all duration-100 active:scale-95 hover:shadow-2xl hover:scale-105"
            onClick={clickCoin}
            onTouchStart={(e) => {
              e.preventDefault();
              clickCoin(e);
